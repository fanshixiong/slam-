...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    Eigen::Quaterniond last_Qwb(Qwb);            // quaterniond:  from imu measurements
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        /// 中值积分
        Qwb = last_Qwb * dq;
        Eigen::Vector3d acc_w = 0.5 * (last_Qwb * last_imupose.imu_acc + Qwb * imupose.imu_acc + 2 * gw); // aw = (Qwb{k-1} * (acc_body-acc_bias) + gw + Qwb{k} * (acc_body-acc_bias) + gw) / 2
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        last_Qwb = Qwb;
